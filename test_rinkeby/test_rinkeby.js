...
describe("Marketplace", function () {
  ...
  before(async function () {
    ...
    asset = await CD.attach(ASSET_ADDRESS);
    ...
    beacon = await CD.attach(BEACON_ADDRESS);
    ...
    lazyTransferProxy = await CD.attach(LAZY_TRANSFER_PROXY_ADDRESS);
    ...
    nftTransferProxy = await CD.attach(NFT_TRANSFER_PROXY_ADDRESS);
    ...
    factory = await CD.attach(FACTORY_ADDRESS);
    console.log("factory Deployed to : ", factory.address);
    collectionProxyAddress = COLLECTION_ADDRESS;
    ...
    erc20TransferProxy = await CD.attach(ERC20_TRANSFER_PROXY_ADDRESS);
    ...
    royaltiesRegistry = await CD.attach(ROYALTIES_ADDRESS);
  ...
  try{
    await exchange.connect(hacker).matchOrders(buyOrder, "0x", sellOrder, sellSignature);
  }catch(e){
    failCounter++;
  }
  ...
  for(let i=0; i<2;i++){
    ...
    console.log("direct buy tx hash => ",receipt.transactionHash, ", status = ",receipt.status);
  }
})
...
it("cancelled orders cant be executed", async()=>{
  ...
  console.log("direct buy before cancelling order, tx hash =>",receipt.transactionHash, ", status = ",receipt.status);

  tx = await exchange.connect(owner).cancel(sellOrder);
  receipt = await tx.wait();
  console.log("direct buy cancel order, tx hash =>",receipt.transactionHash, ", status = ",receipt.status);
...
it("accept bids", async()=>{
  ...
  try{ 
    await exchange.connect(hacker).matchOrders(sellOrder, "0x", buyOrder, buySignature);
  }catch(e){
    failCounter++;
  }
  ...
  receipt = await tx.wait(1);
  console.log("accept bid, tx hash =",receipt.transactionHash, ", status = ",receipt.status);
...
it("accept timed bids", async()=>{
  ...
  tx = await exchange.connect(owner).matchOrders( buyOrder, buySignature, sellOrder, sellSignature );
  receipt = await tx.wait();
  console.log("accept timed bid, tx hash =", receipt.transactionHash, ", status = ",receipt.status);


  /* Accepting bids after the bid end time (should fail) */
  let currentTime;
  let prevBlockNumber, prevBlock, prevBlockTime;
  while(true){
    prevBlockNumber = await ethers.provider.getBlockNumber();
    prevBlock = await ethers.provider.getBlock(prevBlockNumber);
    prevBlockTime = prevBlock.timestamp;
    if(prevBlockTime >= timestampBefore+TIME_OFFSET) break;
    await timeout(2000);
  }
...
it("ETH as fees", async()=>{
  ...
  receipt = await tx.wait(1);
  console.log("buy using ETH, tx hash =",receipt.transactionHash, ", status = ",receipt.status);
})
...
it("set movie details", async()=>{
  ...
  receipt = await tx.wait(1);
  console.log("set movie details, tx hash =",receipt.transactionHash, ", status = ",receipt.status);


  let name = await collectionProxy.connect(addr1).name();
  // name.wait(1);
  assert.equal(name, MOVIE_COLLECTION_NAME);
})
...
it("sets token uri", async()=>{
  ...
  try{
    await collectionProxy.connect(hacker).updateTokenURI(tokenId, revealTokenUri, "0x");
  }catch(e){
    failCounter++;
  }
  ...
  try{
   await collectionProxy.connect(producerUser).updateTokenURI(tokenId, revealTokenUri, producerSignature);
  }catch(e){
    failCounter++;
  }
  ...
  try{
    await collectionProxy.updateTokenURI(tokenId, revealTokenUri, ownerSignature);
  }catch(e){
    failCounter++;
  }
  ...
  try{
    await collectionProxy.updateTokenURI(tokenId, revealTokenUri, "0x");
  }catch(e){
  failCounter++;
  }
  ...
  receipt = await tx.wait(1);
  console.log("sets token uri, tx hash =",receipt.transactionHash, ", status = ",receipt.status);
...
it("sets movie producer", async()=>{
  // only owner can set
  try{
    await collectionProxy.connect(hacker).setMovieProducer(tokenId,hacker.address);
  }catch(e){
    failCounter++;
  }
  ...
  try{
    await collectionProxy.setMovieProducer(tokenId,"0x0000000000000000000000000000000000000000");
  }catch(e){
    failCounter++;
  }

  tx = await collectionProxy.connect(owner).setMovieProducer(tokenId,addr1.address);
  receipt = await tx.wait(1);
  console.log("sets movie producer, tx hash =",receipt.transactionHash, ", status = ",receipt.status);
})

it("sets Movie Reveal Time", async()=>{
  try{
    await collectionProxy.connect(hacker).setMovieRevealTime(tokenId,"12345");
  }catch(e){
    failCounter++;
  }
