...
abstract contract ERC1155Base is OwnableUpgradeable, ERC1155DefaultApproval, ERC1155BurnableUpgradeable, ERC1155Lazy, HasContractURI {
    ...
    function updateTokenURI(uint256 tokenId, string memory newTokenURI, bytes calldata producerSignature) external onlyOwner{
        uint256 movieId = tokenIdToMovieId[tokenId];
    ...
    function _setMovieProducer(uint256 _movieId, address _producer) external onlyOwner {
        require(_producer != address(0), "Producer can't be zero address");
        movieIdToProducer[_movieId] = _producer;
    }

    function _setMovieIdRevealTime(uint tokenId, uint256 _revealTime) external onlyOwner {
        nftRevealTime[tokenId] = _revealTime;
    }
