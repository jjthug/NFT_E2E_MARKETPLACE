...
abstract contract ERC1155Lazy is IERC1155LazyMint, ERC1155BaseURI, Mint1155Validator, RoyaltiesV2Upgradeable, RoyaltiesV2Impl {
    ...
    /* Movie Reveal time */
    mapping(uint256 => uint256) public nftRevealTime;
    /* Token id to Movie id */
    mapping(uint256 => uint256) public tokenIdToMovieId;
    /* Producer address of movie id */
    mapping(uint256 => address) public movieIdToProducer;
    ...
    function _setTokenIdToMovieId(uint256 tokenId, uint256 movieId) internal {
        tokenIdToMovieId[tokenId] = movieId;
    }

    function _setNFTRevealTime(uint256 movieId, uint256 time) internal {
        nftRevealTime[movieId] = time;
    }

    function _setProducer(uint256 movieId, address producer) internal {
