...
contract RoyaltiesRegistry is IRoyaltiesProvider, OwnableUpgradeable {
    ...
    function getRoyalties(address token, uint tokenId) override external returns (LibPart.Part[] memory) {
        ...
        //case royaltiesType = 2, royalties Moviecoin v2
        if (royaltiesType == 2) {
            return getRoyaltiesMoviecoinV2(token,tokenId);
        }

        //case royaltiesType = 3, royalties Moviecoin v1
        if (royaltiesType == 3) {
            return getRoyaltiesMoviecoinV1(token, tokenId);
        }
    ...
    /// @dev tries to get royalties Moviecoin-v2 for token and tokenId
    function getRoyaltiesMoviecoinV2(address token, uint tokenId) internal view returns (LibPart.Part[] memory) {
        try RoyaltiesV2(token).getMoviecoinV2Royalties(tokenId) returns (LibPart.Part[] memory result) {
            return result;
        } catch {
            return new LibPart.Part[](0);
        }
    ...
    /// @dev tries to get royalties Moviecoin-v1 for token and tokenId
    function getRoyaltiesMoviecoinV1(address token, uint tokenId) internal view returns (LibPart.Part[] memory) {
